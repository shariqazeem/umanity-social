...
const CONVERSATIONAL: ConversationalPattern[] = [
  // Greetings
  {
    ...
    response: ({ userStats }) => {
      ...
      return `Hey${name}! I'm RISEN AI, your impact advisor. I can help you find the right cause to donate to, explain how our escrow system works, show you Blink URLs to share, or answer anything about the platform. What's on your mind?`
    },
  },
  // How are you / personal questions
  {
    ...
    response: ({ userStats }) => {
      const name = userStats?.username ? `, @${userStats.username}` : ''
      return `I'm doing great${name}! Always energized when I get to help people make an impact. I'm RISEN's AI advisor — built to know everything about our 6 charity pools, escrow system, governance, NFTs, and more. What can I help you with today?`
    },
  },
  // Thanks
  {
  ...
  // Yes / No / Ok
  {
    ...
    response: `Glad to hear that! Is there anything else you'd like to know about RISEN? I can help with pool recommendations, donation steps, Blink URLs, governance, NFTs, or anything else.`,
  },
  // Wow / reactions
  {
    test: (s) => /^(wow|whoa|amazing|incredible|impressive|that's cool|that's awesome|love it|love this)/i.test(s),
    response: `Right?! RISEN is building something special — on-chain transparency, community governance, and real impact. If you want to dive deeper into any feature or need help with anything, just ask!`,
  },
  ...
  for (const [keyword, poolId] of Object.entries(POOL_IDS)) {
    if (lower.includes(keyword)) {
      const meta: Record<string, string> = {
        ...
        'orphanage-aid': 'Local Orphanage Aid — supplies & education for orphanages, personally delivered by RISEN. Donate: /api/actions/donate/orphanage-aid',
        'animal-rescue': 'Street Animal Rescue — rescue, shelter & medical care, personally delivered by RISEN. Donate: /api/actions/donate/animal-rescue',
      }
      return meta[poolId] || ''
    }
  }
...
export function RisenAgent({ poolStats, userStats }: RisenAgentProps) {
  ...
  useEffect(() => {
    const welcomeContent = userStats?.username
      ? `Hey @${userStats.username}! I'm RISEN AI — your impact advisor. I can help you find the right cause, explain how our escrow works, or answer anything about the platform. What's on your mind?`
      : `Welcome to RISEN! I'm your AI impact advisor. I can help you discover causes, understand how on-chain donations work, or guide you through the platform. What would you like to know?`
          ...
          >
            ...
            <div className="bg-[#111] px-5 py-4 flex items-center gap-3 flex-shrink-0">
              ...
              <div className="flex-1">
                <p className="text-white text-sm font-semibold">RISEN AI</p>
